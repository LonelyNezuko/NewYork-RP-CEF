...
export default function Admin()
{
    ...
    const [ menu, setMenu ] = React.useState('addproperty')
    ...
    const [ frac, setFrac ] = React.useState([
        { id: 0, name: "Los Santos Police Deportament", desc: "Полицейский участок Лос Сантоса", leader: "Nezuko Kamado", status: "Онлайн" },
        { id: 0, name: "Los Santos Police Deportament", desc: "Полицейский участок Лос Сантоса", leader: "Отсутствует", status: "Неизвестно" },
        { id: 0, name: "Los Santos Police Deportament", desc: "Полицейский участок Лос Сантоса", leader: "Nezuko Kunis", status: "Offline" }
    ])
                    ...
                    <div className="admin-cmd admin-reports">
                        {cmd.map((item, i) =>
                        {
                            return (<section key={i} className="admin-reports-item" id={`admin-cmd-item-${i}`}>
                                ...
                                {
                                    return (<div className="admin-reports-item-elem">
            ...
            <section className="admin-body" style={menu !== 'frac' ? {display: 'none'} : {}}>
                ...
                <div className="admin-body-wrap">
                    <div className="admin-cmd admin-reports">
                        {frac.map((item, i) =>
                        {
                            return (<section key={i} className="admin-reports-item" id={`admin-frac-item-${i}`}>
                                <div className="admin-reports-item-elem">
                                    <h1>{item.name}</h1>
                                    <h2>{item.desc}</h2>
                                </div>
                                <div className="admin-reports-item-elem">
                                    <h1 style={item.leader === 'Отсутствует' ? {color: '#ED4343'} : {}}>{item.leader}</h1>
                                    <h2>Кто сейчас лидер?</h2>
                                </div>
                                <div className="admin-reports-item-elem">
                                    <h1 style={item.leader === 'Отсутствует' ? {color: '#ED4343'} : item.status === 'Offline' ? {color: '#ffffff36'} : {}}>{item.status}</h1>
                                    <h2>Статус лидера</h2>
                                </div>
                                <div className="admin-reports-item-elem">
                                    <button onClick={() => fracSend(item.id)} className="btn">Опции</button>
                                </div>
                            </section>)
                        })}
                    </div>
                    ...
                    <div className="admin-chat admin-admins">
                        <section className="admin-chat-players">
                            <div className="admin-chat-players-body">
                                {admins.map((item, i) =>
                                {
                                    return (<section id={`admin-admins-players-${i}`} onClick={() => setAdminsSelect(i)} className={adminsSelect === i && "admin-admins-players-select"} key={i}>
