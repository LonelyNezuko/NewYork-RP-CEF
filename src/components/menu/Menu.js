...
export default function Menu()
{
    ...
    const [ faqData, setFAQData ] = React.useState([
        ...
        { name: "Заголовок 2", desc: "Какое-то очень очень очень очень очень очень очень очень очень очень очень очень очень очень очень очень очень очень очень очень очень очень очень очень очень очень длинное описание" }
    ])
    const [ faqSelect, setFAQSelect ] = React.useState(-1)

    const [ reportData, setReportData ] = React.useState([
        { name: "Памогите пжлст", status: 0, date: new Date(), desc: "У меня украли машину, памагите!!!", messages: [
        ...
        { name: "Памогите пжлст", status: 1, date: new Date(), desc: "У меня украли машину, памагите!!!", messages: [
            { name: "Nezuko Kamado", text: "У меня украли машину, памагите!!!", type: "player", date: new Date() },
            { name: "Nezuko Kun", text: "Уже лечу вам памагать", type: "admin", date: new Date() }
        ] }
    ])
    ...
    const [ questsData, setQuestsData ] = React.useState([
        { name: 'Название квеста #1', desc: "Описание квеста", prize: '$650.000.000 и уникальный аксессуар', status: false, traking: true, tasks: [
        ...
        { name: 'Название квеста #2', desc: "Описание квеста", prize: '360 Доната', status: true, traking: false, tasks: [
            { name: 'Какая-то задача #1', count: [ 1, 1 ] }
        ] },
        { name: 'Название квеста #3', desc: "Описание квеста", prize: 'Чета', status: false, traking: false, tasks: [
            { name: 'Какая-то задача #1', count: [ 1, 2 ] }
        ] }
    ])
    ...
    const [ rewardsData, setRewardsData ] = React.useState([
        ...
        { name: "Название реварда #3", desc: "Описание реварда", prize: "250 доната, уникальный аксессуар и $650.000.000", count: [ 250, 250 ] }
    ])
    ...
    const [ settingsData, setSettingsData ] = React.useState({
        ...
        promo: { promo: '#NEZUKOKUN', players: 0, level: "0001", bonuse: [
    ...
    const [ donateVipData, setDonateVipData ] = React.useState([
        { name: "Bronze", desc: "Самая минимальная випка", price: 150, img: "donate_vip_1.png", tasks: [
            [ 'Наличные', '$65.000' ],
            [ 'Донат', '350' ]
        ] }
    ])
    ...
    function openRoullete(fast = false)
    {
        ...
        $('#menu-donate-roullete-section').animate({
            scrollLeft: $(`.menu-donate-roulette-item-anim[data-id='${rand}']`).position().left - $('#menu-donate-roullete-section')[0].clientWidth / 2 + $(`.menu-donate-roulette-item-anim[data-id='${rand}']`)[0].clientWidth / 2 + 6
        }, !fast ? 15000 : 0, () =>
        {
        ...
        <div className="menu" style={toggle === false ? {display: 'none'} : {}}>
            <header>
                ...
                <section className="menu-header-nav">
                    <div>
                        <button onClick={() => openMenu('stats')} className={`btn-ui ${menu === 'stats' && 'btn-ui-select'}`}>Статистика</button>
                        <button onClick={() => openMenu('skills')} className={`btn-ui ${menu === 'skills' && 'btn-ui-select'}`}>Навыки</button>
                        <button onClick={() => openMenu('donate')} className={`btn-ui ${menu.indexOf('donate') !== -1 && 'btn-ui-select'}`}>Магазин</button>
                        <button onClick={() => openMenu('faq')} className={`btn-ui ${menu === 'faq' || menu === 'report' ? 'btn-ui-select' : ''}`}>FAQ</button>
                        <button onClick={() => openMenu('rewards')} className={`btn-ui ${menu === 'rewards' && 'btn-ui-select'}`}>Достижения</button>
                        <button onClick={() => openMenu('settings')} className={`btn-ui ${menu.indexOf('settings') !== -1 && 'btn-ui-select'}`}>Настройки</button>
                        <button onClick={() => openMenu('quests')} className={`btn-ui ${menu === 'quests' && 'btn-ui-select'}`}>Квесты</button>
                    </div>
                    <div className="menu-header-nav-two" style={menu !== 'faq' && menu !== 'report' ? {display: 'none'} : {}}>
                        <button onClick={() => openMenu('report')} className={`btn-ui ${menu === 'report' && 'btn-ui-select'}`}>Репорт</button>
                        <button onClick={() => { setCreateReport(true); openMenu('report') }} className={`btn-ui`} style={{width: "auto", "clip-path": "polygon(0% 0%, 92% 0%, 100% 100%, 8% 100%)"}}>Написать новый репорт</button>
                    </div>
                    <div className="menu-header-nav-two" style={menu.indexOf('settings') === -1 ? {display: 'none'} : {}}>
                        <button onClick={() => openMenu('settings')} className={`btn-ui ${menu === 'settings' && 'btn-ui-select'}`}>Аккаунт</button>
                        <button onClick={() => openMenu('settings-email')} className={`btn-ui ${menu === 'settings-email' && 'btn-ui-select'}`}>Эл. почта</button>
                        <button onClick={() => openMenu('settings-interface')} className={`btn-ui ${menu === 'settings-interface' && 'btn-ui-select'}`}>Интерфейс</button>
                        <button onClick={() => openMenu('settings-promo')} className={`btn-ui ${menu === 'settings-promo' && 'btn-ui-select'}`}>Партнерство</button>
                        <button onClick={() => openMenu('settings-aim')} className={`btn-ui ${menu === 'settings-aim' && 'btn-ui-select'}`}>Прицел</button>
                        <button onClick={() => openMenu('settings-chat')} className={`btn-ui ${menu === 'settings-chat' && 'btn-ui-select'}`}>Чат</button>
                        <button onClick={() => openMenu('settings-keys')} className={`btn-ui ${menu === 'settings-keys' && 'btn-ui-select'}`}>Клавиши</button>
                    </div>
                    <div className="menu-header-nav-two" style={menu.indexOf('donate') === -1 ? {display: 'none'} : {}}>
                        <button onClick={() => openMenu('donate')} className={`btn-ui ${menu === 'donate' && 'btn-ui-select'}`}>Рулетка</button>
                        <button onClick={() => openMenu('donate-cash')} className={`btn-ui ${menu === 'donate-cash' && 'btn-ui-select'}`}>Валюта</button>
                        <button onClick={() => openMenu('donate-cars')} className={`btn-ui ${menu === 'donate-cars' && 'btn-ui-select'}`}>Транспорт</button>
                        <button onClick={() => openMenu('donate-vip')} className={`btn-ui ${menu === 'donate-vip' && 'btn-ui-select'}`}>Вип</button>
                        <button onClick={() => openMenu('donate-other')} className={`btn-ui ${menu === 'donate-other' && 'btn-ui-select'}`}>Остальное</button>
                    </div>
                </section>
            </header>
            <section className="menu-body">
                <section className="menu-wrap menu-stats" style={menu !== 'stats' ? {display: 'none'} : {}}>
                    ...
                    <div className="menu-section">
                        <div className="menu-stats-body">
                            ...
                            <div>
                                <h1>Аккаунт</h1>
                                <h2>{statsData.userName} [{statsData.userID}]</h2>
                            </div>
                            <div>
                                <h1>Игровой уровень</h1>
                                <h2>{statsData.level[0]} LVL [ {statsData.level[1]}/{statsData.level[2]} EXP ]</h2>
                            </div>
                            <div>
                            ...
                            <div>
                                ...
                                <h2>{statsData.married === 'None' ? `Не ${statsData.gender === 0 ? 'женат' : 'замужем'}` : `${statsData.gender === 0 ? 'Женат' : 'Замужем'} на ${statsData.married}`}</h2>
                            </div>
                            <div>
                        ...
                        <div className="menu-stats-body menu-stats-property">
                            ...
                            <img className="menu-stats-property-title-img" src={IMG_STATS_PROPERTY_IMG} />

                            <section className="menu-stats-property-wrap">
                                {statsData.property.map((item, i) =>
                                {
                                    return (<div className={`menu-stats-property-${item.type}`}>
                                        <h2>
                                            {item.type === 'house' ? 'Жилье' : item.type === 'vehicle' ? 'Транспорт $' + item.id : 'Бизнес'}
                                            <span>{item.name}</span>
                                        </h2>
                                        <h2>
                ...
                <section className="menu-wrap menu-skills" style={menu !== 'skills' ? {display: 'none'} : {}}>
                    ...
                    <div className="menu-section">
                        ...
                        {
                            return (<div key={i} className="menu-skills-item">
                ...
                <section className="menu-wrap menu-faq" style={menu !== 'faq' ? {display: 'none'} : {}}>
                    <div className="menu-section">
                        <section className="menu-faq-themes">
                            {faqData.map((item, i) =>
                            {
                                return (<div onClick={() => setFAQSelect(i)} key={i} className={faqSelect === i && "menu-faq-themes-select"}>
                                    <section>
                                        <h1>{item.name}</h1>
                                        <span>{item.desc.substring(0, 58)}{item.desc.length > 58 ? "..." : ""}</span>
                                    </section>
                                    <button></button>
                                </div>)
                            })}
                        ...
                        <section className="menu-faq-body">
                            ...
                            <div className="menu-faq-body-report" style={faqSelect === -1 ? {display: 'none'} : {}}>
                                ...
                                <div>
                                    <button onClick={() => openMenu('report')} className="btn">Написать админу</button>
                                    <img src={IMG_FAQ} />
                                </div>
                            </div>
                        </section>
                ...
                <section className="menu-wrap menu-faq menu-report" style={menu !== 'report' ? {display: 'none'} : {}}>
                    <div className="menu-section">
                        <section className="menu-faq-themes">
                            ...
                            {
                                return (<div onClick={() => setReportSelect(i)} key={i} className={reportSelect === i && "menu-faq-themes-select"}>
                                    <section>
                                        <h1>
                                            {item.name} <h2>{new Date(item.date).toLocaleString('ru-RU')}</h2>
                                        </h1>
                                        <h2 style={{"margin-bottom": "7px"}}><span className={item.status === 0 ? "menu-report-themes-open" : item.status === 1 ? "menu-report-themes-wait" : ""}>{item.status === 0 ? "Открыт" : item.status === 1 ? 'На рассмотрении' : "Закрыт"}</span></h2>
                                        <span>{item.desc.substring(0, 58)}{item.desc.length > 58 && "..."}</span>
                                    </section>
                                    <button></button>
                                </div>)
                        ...
                        <div className="menu-report-wrap">
                            <section className="menu-faq-body" style={reportSelect === -1 ? {display: "none"} : {}}>
                                ...
                                <div className="menu-report-body-chat">
                                    {reportSelect !== -1 && reportData[reportSelect].messages.map((item, i) =>
                                    {
                                        return (<section key={i} className={item.type === 'player' && "menu-report-body-chat-reverse"}>
                                ...
                                <div className="menu-report-body-input" style={reportSelect !== -1 && reportData[reportSelect].status === 2 ? {display: 'none'} : {}}>
                                    <input onKeyDown={e => sendReport(reportSelect, e)} id="menu-report-body-input" type="text" placeholder="Введите Ваше сообщение..." className="input" />
                                    <button onClick={() => closeReport(reportSelect)} className="btn">Моя проблема решена</button>
                                    <button onClick={() => sendReport(reportSelect)} className="btn btn-select">►</button>
                                </div>
                            </section>
                        </div>
                    </div>
                    <div className="menu-report-create" style={createReport === false ? {display: 'none'} : {}}>
                        <div className="menu-report-create-wrap">
                            <h1>Написать в репорт <button onClick={() => setCreateReport(false)}>✖</button></h1>
                            <section>
                                ...
                                <br />
                                <br />
                                <center style={{color: "#ED4343"}}>Помните! Администрация никогда не попросит у Вас Ваших личных данных, паролей от аккаунтов и пр.</center>
                            </section>
                            <input onKeyDown={e => createReportSend(e)} id="menu-report-create" type="text" placeholder="Введите Ваше сообщение..." className="input" />
                            <div className="menu-report-create-btn">
                                <button onClick={() => createReportSend()} className="btn">Отправить</button>
                            </div>
                        </div>
                    </div>
                </section>
                <section className="menu-wrap menu-quests" style={menu !== 'quests' ? {display: 'none'} : {}}>
                    <div className="menu-quests-wrap">
                        ...
                        {
                            return (<div key={i} className="menu-quests-item">
                                <div className="menu-quests-item-title">
                                    <h1 className={item.traking === true && "menu-quests-item-traking"}><h3 onClick={() => trackQuest(i)}>★</h3> {item.name}</h1>
                                    <span>{item.desc}</span>
                                    <h2>Награда: <span>{item.prize}</span></h2>
                                </div>
                                <div className="menu-quests-item-tasks">
                                    {item.tasks.map((tasks, task) =>
                                    {
                                        return (<div key={task}>
                                            <h1>{tasks.name}</h1>
                                            <h2 className={tasks.count[0] >= tasks.count[1] && 'menu-quests-item-tasks-yes'}>{tasks.count[0] >= tasks.count[1] ? 'Выполненно' : `${tasks.count[0]} / ${tasks.count[1]}`}</h2>
                                        </div>)
                                    })}
                                </div>
                                <button onClick={() => compilQuest(i)} className={`btn ${item.status === true && 'btn-select'}`}>Забрать награду</button>
                            </div>)
                        })}
                    </div>
                </section>
                <section className="menu-wrap menu-quests menu-rewards" style={menu !== 'rewards' ? {display: 'none'} : {}}>
                    <div className="menu-quests-wrap">
                        {rewardsData.map((item, i) =>
                        {
                            ...
                            return (<div key={i} className="menu-quests-item menu-quests-item-s">
                                <div className="menu-quests-item-title">
                                    <h1 className="menu-quests-item-traking"><h3>★</h3> {item.name}</h1>
                                    <span>{item.desc}</span>
                                    <h2>Награда: <span>{item.prize}</span></h2>
                                </div>
                                <button>
                                    <svg viewBox="0 0 50 50">
                                        <circle cx="25" cy="25" r="20" fill="none" stroke-width="2" style={{"stroke-dashoffset": -126 + prox}}>
                                        </circle>
                                    </svg>
                                    <h1>{item.count[0]} <span>/</span> {item.count[1]}</h1>
                                </button>
                            </div>)
                        })}
                    </div>
                    ...
                    <div className="menu-quests-wrap">
                        {settingsData.account.map((item, i) =>
                        {
                            return (<div key={i} className="menu-quests-item menu-quests-item-s">
                                ...
                                <section>
                                    <button onClick={() => { let tempData = settingsData.account; tempData[i].status = true; setSettingsData({ ...settingsData, account: tempData }); ragemp.send('ui::menu:updateSettings', tempData) }} className={`btn btn-visual ${item.status === true && 'btn-select'}`}>Да</button>
                                    <button onClick={() => { let tempData = settingsData.account; tempData[i].status = false; setSettingsData({ ...settingsData, account: tempData }); ragemp.send('ui::menu:updateSettings', tempData) }} className={`btn btn-visual btn-visual-reverse ${item.status === false && 'btn-select'}`}>Нет</button>
                                </section>
                            </div>)
                        })}
                    </div>
                    ...
                    <div className="menu-quests-wrap">
                        {settingsData.interface.map((item, i) =>
                        {
                            return (<div key={i} className="menu-quests-item menu-quests-item-s">
                                ...
                                <section>
                                    <button onClick={() => { let tempData = settingsData.interface; tempData[i].status = true; setSettingsData({ ...settingsData, interface: tempData }); ragemp.send('ui::menu:updateSettings', tempData) }} className={`btn btn-visual ${item.status === true && 'btn-select'}`}>Да</button>
                                    <button onClick={() => { let tempData = settingsData.interface; tempData[i].status = false; setSettingsData({ ...settingsData, interface: tempData }); ragemp.send('ui::menu:updateSettings', tempData) }} className={`btn btn-visual btn-visual-reverse ${item.status === false && 'btn-select'}`}>Нет</button>
                                </section>
                            </div>)
                        })}
                    </div>
                </section>
                <section className="menu-wrap menu-quests menu-settings" id="menu-settings-keys" data-keys={JSON.stringify(settingsData.keys)} data-id={settingsDataChangeKeys} style={menu !== 'settings-keys' ? {display: 'none'} : {}}>
                    <div className="menu-quests-wrap">
                        {settingsData.keys.map((item, i) =>
                        {
                            return (<div key={i} className="menu-quests-item menu-quests-item-s menu-settings-item-key">
                                <h1>
                                    {item.name}
                                    <h2>{item.desc}</h2>
                                </h1>
                                <section>
                                    <button onClick={() => {setSettingsDataChangeKeys(settingsDataChangeKeys === i ? -1 : i)}} className={`btn btn-visual btn-visual-reverse ${settingsDataChangeKeys === i && 'btn-select'}`}>{settingsDataChangeKeys === i ? "Нажмите на клавишу" : item.key}</button>
                                </section>
                            </div>)
                        })}
                    </div>
                ...
                <section className="menu-wrap menu-quests menu-settings-promo" style={menu !== 'settings-promo' ? {display: 'none'} : {}}>
                    <div className="menu-quests-item menu-quests-item-s">
                        ...
                        <h2>Приглашено людей: <span>{settingsData.promo.players} человека</span></h2>
                ...
                <section className="menu-wrap menu-quests menu-settings-promo menu-settings-aim" style={menu !== 'settings-aim' ? {display: 'none'} : {}}>
                    <div className="menu-quests-wrap">
                        <div className="menu-quests-item menu-quests-item-s">
                            <h3>Прицел</h3>
                            <div className="menu-settings-aim-body menu-settings-aim-body-point" style={settingsData.crosshair.type !== 'point' ? {display: 'none'} : {}}>
                                <button style={{width: settingsData.crosshair.size + "px", height: settingsData.crosshair.size + "px"}}></button>
                            </div>
                            <div className="menu-settings-aim-body menu-settings-aim-body-standart" style={settingsData.crosshair.type !== 'standart' ? {display: 'none'} : {}}>
                                <button></button>
                                <button></button>
                                <button></button>
                                <button></button>
                            ...
                            <div className="menu-settings-aim-param">
                                ...
                                <Choiceinput data={{ select: settingsData.crosshair.type === 'standart' ? 0 : 1, params: ['Стандартный', 'Точка'], onChange: (value) => setSettingsData({...settingsData, crosshair: {...settingsData.crosshair, type: value === 0 ? "standart" : "point"}}) }} />
                            </div>
                            <div className="menu-settings-aim-param">
                            ...
                            <h2 style={{"margin-top": "30px"}}>Только истинный гетто-тащер сможет подобрать для себя крутой прицел, а ты - гетто тащер?</h2>
                        </div>
                        <div className="menu-quests-item menu-quests-item-s">
                            ...
                            <div className="menu-settings-aim-param">
                                ...
                                <input className="input-range input-range-visual" min="2" max="15" type="range" value={settingsData.crosshair.size} onChange={e => setSettingsData({...settingsData, crosshair: {...settingsData.crosshair, size: e.target.value}})} />
                            </div>
                            <div className="menu-settings-aim-param" style={settingsData.crosshair.type !== 'standart' ? {display: 'none'} : {}}>
                                <h1>Ширина прицела</h1>
                                <h2>Чем шире прицел, тем лучше стрелять. Так ведь?</h2>
                                <input className="input-range input-range-visual" min="0" max="10" type="range" value={settingsData.crosshair.height} onChange={e => setSettingsData({...settingsData, crosshair: {...settingsData.crosshair, height: e.target.value}})} />
                            </div>
                            <div className="menu-settings-aim-param">
                ...
                <section className="menu-wrap menu-donate" style={menu !== 'donate' ? {display: 'none'} : {}}>
                    <div className="menu-donate-roulette">
                        ...
                        <section id="menu-donate-roullete-section">
                            ...
                            {
                                return (<div key={i} className="menu-donate-roulette-item menu-donate-roulette-item-anim" data-id={i}>
                                    <img src={require(`./images/roulette/${item.img}`)} />
                                    <h1>{item.name}</h1>
                        ...
                        <div className="menu-donate-roulette-down">
                            <button onClick={() => ragemp.send('ui::menu:roullete:go', { fast: true })} className="btn btn-visual">Открыть быстро</button>
                            <button onClick={() => ragemp.send('ui::menu:roullete:go', { fast: false })} className="btn btn-select">Крутить</button>
                            <button className="btn btn-visual btn-visual-reverse">350 GC</button>
                            <h5 className="menu-donate-value">Ваше количество GC: <span>{donateCount.toLocaleString()}</span></h5>
                        </div>
                    </div>
                    <div className="menu-donate-roulette-prizes">
                        ...
                        <section>
                            {donateRoullete.prizes.map((item, i) =>
                            {
                                return (<div key={i} className="menu-donate-roulette-item">
                                    <img src={require(`./images/roulette/${item.img}`)} />
                                    <h1>{item.name}</h1>
                                    <h2 className={`menu-donate-roulette-item-type-${item.type}`}>{item.type}</h2>
                                </div>)
                ...
                <section className="menu-wrap menu-quests menu-donate menu-donate-cash" style={menu !== 'donate-cash' ? {display: 'none'} : {}}>
                    <div className="menu-quests-wrap">
                        {donateCashData.map((item, i) =>
                        {
                            return (<div key={i} className="menu-quests-item menu-quests-item-s">
                                ...
                                <h2>{item.desc}</h2>
                                <section>
                                    <img src={require(`./images/donate_cash/${item.img}`)} />
                                    <h3>
                                ...
                                <div>
                                    <button onClick={() => donateBuyCash(i)} className="btn btn-select">Купить</button>
                ...
                <section className="menu-wrap menu-quests menu-donate menu-donate-cash menu-donate-cars" style={menu !== 'donate-cars' ? {display: 'none'} : {}}>
                    <div className="menu-quests-wrap">
                        {donateCarsData.map((item, i) =>
                        {
                            return (<div key={i} className="menu-quests-item menu-quests-item-s">
                                ...
                                <h2>{item.desc}</h2>
                                <section>
                                    <img src={require(`./images/donate_cars/${item.img}`)} />
                                </section>
                                <div>
                                    <button onClick={() => donateCarsBuy(i)} className="btn btn-select">Купить</button>
                                    <button className="btn btn-visual btn-visual-reverse">{item.price} GC</button>
                                    <h5 className="menu-donate-value">Ваше количество GC: <span>{donateCount.toLocaleString()}</span></h5>
                                </div>
                            </div>)
                ...
                <section className="menu-wrap menu-quests menu-donate menu-donate-cash menu-donate-cars menu-donate-vip" style={menu !== 'donate-vip' ? {display: 'none'} : {}}>
                    <div className="menu-quests-wrap">
                        {donateVipData.map((item, i) =>
                        {
                            return (<div key={i} className="menu-quests-item menu-quests-item-s">
                                ...
                                <section>
                                    <section>
                                        <img src={require(`./images/donate_vip/${item.img}`)} />
                                    </section>
                                    <div className="menu-quests-item-tasks">
                                ...
                                <div>
                                    <button onClick={() => donateVipBuy(i)} className="btn btn-select">Купить</button>
                                    <button className="btn btn-visual btn-visual-reverse">{item.price} GC</button>
                                    <h5 className="menu-donate-value">Ваше количество GC: <span>{donateCount.toLocaleString()}</span></h5>
                                </div>
                            </div>)
                ...
                <section className="menu-wrap menu-donate menu-quests menu-donate-other" style={menu !== 'donate-other' ? {display: 'none'} : {}}>
                    ...
                    <div className="menu-donate-other-code">
                        <div className="menu-quests-item menu-quests-item-s">
                            <h1>Бонус код</h1>
                            <h2>Описание прямо тут</h2>
                            <img src={require(`./images/donate_other_code.png`)} />
